...
from .records import SampleRecord, load_jsonl, save_jsonl
...
def _canonical_keys_from_csv_path(raw_path: str, default_split: str | None) -> list[str]:
    ...
    if modality_index is None:
        ...
        modality = MODALITY_ALIASES[lower_parts[modality_index]]
...
def _find_mask_file(mask_root: Path, rel_path: Path) -> Path | None:
    ...
    candidates = sorted(
        path for path in parent.glob(f"{stem}*") if path.is_file() and is_image_file(path)
    )
    if not candidates:
        return None

    illusion_candidates = [path for path in candidates if "illusion" in path.stem.lower()]
    ...
    for split_root in split_roots:
        ...
        for required_root, modality in (
            ...
            if not required_root.exists():
                raise FileNotFoundError(f"Missing {modality} directory: {required_root}")

        right_files = sorted(
            path for path in right_root.rglob("*") if path.is_file() and is_image_file(path)
        )
        if not right_files:
            raise FileNotFoundError(f"No right-view image files found under {right_root}")
        ...
        for right_path in right_files:
            ...
            mask_path = _find_mask_file(mask_root, rel_path) if mask_root.exists() else None

            if left_path is None:
                raise FileNotFoundError(f"Missing matching left image for {right_path}")
            if depth_path is None:
                raise FileNotFoundError(f"Missing matching depth proxy for {right_path}")
            if require_mask and mask_path is None:
                missing_masks.append(str(right_path))

            scale_factor = scale_lookup.resolve(split_name=split_name, right_rel_path=rel_path)
