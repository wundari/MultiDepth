...
class IllusionDepthDataset(Dataset):
    """PyTorch dataset for extracted 3D Visual Illusion training samples.

    Returned sample contract:
    - left/right: FloatTensor[3,H,W], RGB values in 0..255.
    - pseudo_disp: FloatTensor[1,H,W], scaled depth proxy interpreted as disparity.
    - target_flow: FloatTensor[1,H,W], negative pseudo_disp for stereo warping code.
    - valid_disp: BoolTensor[1,H,W], pseudo_disp > 0.
    - illusion_mask: BoolTensor[1,H,W] or None.
    - valid: BoolTensor[1,H,W], valid_disp optionally restricted to the illusion mask.
    """
    ...
    def _check_shapes(
        ...
        """Fail early if a sample's modalities do not share height and width."""
        h_w = left.shape[:2]
