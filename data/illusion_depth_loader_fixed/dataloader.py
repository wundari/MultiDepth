...
from .hf_index import build_hf_training_index, load_index_jsonl
...
class StereoBatchCollator:
    ...
    def _pad_tensor(
        ...
        if tensor.ndim != 3:
            raise ValueError(f"Expected [C,H,W] tensor, got shape {tuple(tensor.shape)}")
    ...
    def __call__(self, samples: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        batch: dict[str, list[torch.Tensor]] = {key: [] for key in _BATCH_TENSOR_KEYS}
        illusion_masks: list[torch.Tensor] = []
        ...
        for sample in samples:
            ...
            for key in _BATCH_TENSOR_KEYS:
                batch[key].append(self._crop_tensor(padded[key], top, left, crop_h, crop_w))
