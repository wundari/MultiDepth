...
from models.mono.depthanything_wrapper import DepthAnythingConfig, FrozenDepthAnythingV2
from models.priors.beta_modulator import (
    BetaModulator,
    BetaModulatorConfig,
    BetaModulationOutput,
)
from models.priors.lbp import LBPConfig, LocalBinaryPattern
...
class RAFTStereoMonoBetaVLMFluxConfig:
    feature_dim: int = 128
    ...
    lbp_neighbor_offsets: tuple[tuple[int, int], ...] = (
        (-1, -1),
        (1, 1),
        (1, -1),
        (-1, 1),
    ...
    vlm_refinement: StereoMonoVLMRefinementConfig = field(
        default_factory=StereoMonoVLMRefinementConfig
    )


class RAFTStereoMonoBetaVLMFluxCore(nn.Module):
    """Stereo + mono + beta + Qwen2VL/Flux-style confidence fusion.

    Returned `disp_predictions` use the released project's negative horizontal-flow
    convention so training remains compatible with the Milestone 1 dataset contract.
    """

    def __init__(self, config: RAFTStereoMonoBetaVLMFluxConfig | None = None) -> None:
        super().__init__()
        cfg = config or RAFTStereoMonoBetaVLMFluxConfig()
        self.config = cfg

        self.feature_encoder = BasicEncoder(cfg.feature_dim, config=EncoderConfig())
        ...
        self.lbp = LocalBinaryPattern(
            LBPConfig(neighbor_offsets=cfg.lbp_neighbor_offsets)
        )
        self.beta_modulator = BetaModulator(
            BetaModulatorConfig(
                lbp_dim=self.lbp.num_neighbors, hidden_dim=None, norm=cfg.norm
            )
        )
        self.refinement = StereoMonoVLMRefinement(cfg.vlm_refinement)
    ...
    def forward(
        ...
        *,
        ...
        left_norm = normalize_uint8_image(
            left
        )  # convert pix val from [0.0, 255.0] to [-1, 1]
        ...
        for iteration in range(num_iters):
            ...
            beta_out = self.beta_modulator(
                disp_lbp, depth_lbp, return_distribution=True
            )
            ...
            delta_flow = torch.cat(
                [delta_flow_x, torch.zeros_like(delta_flow[:, 1:2])], dim=1
            )
        ...
        if beta_out is None or up_mask is None:
            raise RuntimeError(
                "Model executed zero iterations; no refinement inputs were produced"
            )
        ...
        depth_registered_neg_up = upsample_flow(
            ...
            factor=self.config.downsample_factor,
            mask_logits=(
                refine_out.up_mask if self.config.use_convex_upsampling else None
            ),
        )
    ...
    def _split_context_outputs(self, context_outputs: list[torch.Tensor]):
        hidden_states = []
        contexts = []
        for out, hidden_dim, context_dim in zip(
            context_outputs, self.config.hidden_dims, self.config.context_dims
        ):
    ...
    def _validate_inputs(self, left: torch.Tensor, right: torch.Tensor) -> None:
        ...
        if left.shape[-2] % factor != 0 or left.shape[-1] % factor != 0:
            raise ValueError(
                "Milestone 7 expects image sizes divisible by the largest recurrent scale. "
